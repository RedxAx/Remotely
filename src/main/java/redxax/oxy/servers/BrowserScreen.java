...
public class BrowserScreen extends Screen {
    ...
    private void resizeBrowser(MCEFBrowser browser) {
        if (width > 100 && height > 100) {
            ...
            browser.resize((int)(browserWidth * minecraftClient.getWindow().getScaleFactor()), (int)(browserHeight * minecraftClient.getWindow().getScaleFactor()));
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if(mouseY <= titleBarHeight + tabBarHeight + 10) {
            ...
            if(!tabClicked) {
                if(mouseX >= x && mouseX <= x + 18 && mouseY >= tabBarY && mouseY <= tabBarEndY) {
