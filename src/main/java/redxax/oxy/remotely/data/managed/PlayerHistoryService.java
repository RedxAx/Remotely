...
public class PlayerHistoryService {
    ...
    public PlayerHistoryService(RebaseAPI api, Path historyDir, Function<String, UUID> nameResolver) {
        this.api = api;
    ...
    public void recordAccessChangeByName(String name, SessionEventType type, String details, long timestamp) {
        UUID uuid = resolve(name);
        if (uuid == null) return;
        recordAccessChange(uuid, name, type, details, timestamp);
    }
