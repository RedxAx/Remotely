...
public final class DesktopReSyncFlowClientState implements ReSyncFlowClientState, ReSyncWorldGenerationState {
    private final FlowManager manager;
    private final ReSyncLuckPermsHolder luckPerms;

    public DesktopReSyncFlowClientState(FlowManager manager, ReSyncLuckPermsHolder luckPerms) {
    ...
    public void onResourceList(String serverId, ReSyncResourceType type, List<String> ids) {
        if (type.isGraph()) {
            manager.applyServerGraphList(serverId, type, ids);
        } else if (type == ReSyncResourceType.GUI) {
            manager.applyServerGuiList(serverId, ids);
        } else if (type == ReSyncResourceType.SCOREBOARD) {
            manager.applyServerScoreboardList(serverId, ids);
        } else if (type == ReSyncResourceType.TAB) {
            manager.applyServerTabList(serverId, ids);
        } else if (type == ReSyncResourceType.CUSTOM_CONTENT) {
            manager.applyServerCustomContentList(serverId, ids);
        } else if (type == ReSyncResourceType.PROJECT_METADATA) {
            manager.applyServerProjectMetadataList(serverId, ids);
        } else {
            manager.applyServerJsonResourceList(serverId, type, ids);
        }
    }
    ...
    public void onFlowError(String serverId, EditorError editorError, List<Map<String, Object>> attributeErrors, String message) {
        if (editorError != null) {
            GraphEditorScreen.handleEditorErrorForServer(serverId, editorError);
        } else if (attributeErrors != null && !attributeErrors.isEmpty()) {
            ContentDesignerScreen.handleAttributeValidationErrorsForServer(serverId, attributeErrors);
        }
    }
    ...
    public void onResourceSaveSucceeded(String serverId, ReSyncResourceType type, String id, long sequence, long revision, String hash) {
        if (type.isGraph() && revision > 0L) {
            manager.markFlowSaved(serverId, type, id, revision, hash);
        } else if (type == ReSyncResourceType.GUI) {
            manager.markGuiSaved(serverId, id);
        } else if (type == ReSyncResourceType.SCOREBOARD) {
            manager.markScoreboardSaved(serverId, id);
        } else if (type == ReSyncResourceType.TAB) {
            manager.markTabSaved(serverId, id);
        } else if (type == ReSyncResourceType.CUSTOM_CONTENT) {
            manager.markCustomContentSaved(serverId, id);
        } else if (type == ReSyncResourceType.PROJECT_METADATA) {
            manager.markProjectMetadataSaved(serverId);
        } else {
            manager.markJsonResourceSaved(serverId, type, id);
        }
    }

    @Override
    public boolean isBackingFlowAcknowledgement(String serverId, ReSyncResourceType type, String id) {
        if (!type.isGraph() || id == null) {
            return false;
        }
        ...
        return manager.getCustomContentForServer(serverId).values().stream().anyMatch(content -> content != null && id.equals(content.getFlowId()));
    }
    ...
    @Override
    public void onPlaceholderPreview(String serverId, String rendered) {
    }
    ...
    public ReSyncSaveTarget failAnySave(String serverId, String title, String message) {
        DesignerSaveNotifications.SaveTarget target = DesignerSaveNotifications.failAnyForServer(serverId, title, message);
    ...
    @Override
    public Integer collaborationColorOverride() {
        return Config.configManager instanceof RemotelyConfigManager config ? config.getCollaborationColorOverride() : null;
    }

    @Override
    public Object luckPerms(ReSyncFlowClient client) {
        return luckPerms == null ? null : luckPerms.get(client);
    }

    @Override
    public void closeIntegrations(ReSyncFlowClient client) {
        if (luckPerms != null) {
            luckPerms.close(client);
        }
    }
    ...
    private void handleData(String serverId, ReSyncResourceType type, Object item) {
        ...
        else if ((type == ReSyncResourceType.TRADE_PROFILE || type == ReSyncResourceType.NPC_DEFINITION || type == ReSyncResourceType.LOOT_TABLE) && item instanceof JsonObject value) manager.handleFocusedJsonResourceDataReceived(serverId, type, value);
    }
    ...
    @FunctionalInterface
    public interface ReSyncLuckPermsHolder {
        Object get(ReSyncFlowClient client);
